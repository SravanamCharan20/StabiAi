...
const AiSuggestions = ({ employeeData, predictionData, loading }) => {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        // console.log('Sending predictionData:', formattedPredictionData);
  ...
  if (loading || isLoading) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-8 flex items-center justify-center p-12">
  ...
  return (
    <div className="mt-8">
      ...
      <Section title="Critical Skills to Develop" description="High-impact skills that align with market demands"
      ...
      <Section title="Priority Actions" description="Immediate steps to strengthen your position"
      ...
      <Section title="Growth Opportunities" description="Strategic moves to advance your career"
      ...
      <Section title="Recommended Actions" description="Steps you can take now to improve your stability and visibility"
        isEmpty={!suggestions.actions.length}>
        {suggestions.actions.map((action, index) => (
          <ActionCard key={index} action={action} index={index} />
        ))}
      ...
      <Section title="Opportunity Insights" description="Growth or transition options aligned with your profile"
        isEmpty={!suggestions.opportunities.length}>
        {suggestions.opportunities.map((opportunity, index) => (
          <OpportunityCard key={index} opportunity={opportunity} index={index} />
        ))}
